<%-- 
    Document   : dnevnik
    Created on : Jun 18, 2013, 9:50:27 PM
    Author     : zoran
--%>
                ...
                <c:if test="${pageContext.request.method=='GET'}">
                    <c:choose>
                        <c:when test="${param.vrsta=='1'}">                       
                            <sql:query dataSource="${konekcija}" var="upit">SELECT * from ztintor_dnevnik</sql:query>
                        </c:when>
                        <c:when test="${param.vrsta=='2'}">
                            <sql:query dataSource="${konekcija}" var="upit">SELECT * from ztintor_dnevnik where pocetak BETWEEN concat(substring('<c:out value="${param.pocetno}"/>',1,13),':',substring('<c:out value="${param.pocetno}"/>',15,2),':',substring('<c:out value="${param.pocetno}"/>',18,2)) AND concat(substring('<c:out value="${param.zavrsno}"/>',1,13),':',substring('<c:out value="${param.zavrsno}"/>',15,2),':',substring('<c:out value="${param.zavrsno}"/>',18,2)) ORDER BY pocetak ASC</sql:query>
                        </c:when>
                    </c:choose>
                    <display:table name="${upit.rows}" pagesize="${param.select}" style="border:1px solid; text-align:center; background-color: #C5FCC0;">
                        <display:column property="zahtjev" title="zahtjev" style="border:1px solid; background-color: #DBD872;"/>
                        <display:column property="pocetak" title="pocetak" style="border:1px; background-color: #F5F5CE;"/>   
                        <display:column property="kraj" title="kraj" style="border:1px solid; background-color: #DBD872;"/>
                        <display:column property="trajanje" title="trajanje" style="border:1px; background-color: #F5F5CE;"/>  
                    </display:table>    
                </c:if>     
            </center>
        </div>
